...
const getRazorpayKey = async (credentials) => {
  try {
    let response = await fetch('/api/razorpay/key', {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
        'Authorization': 'Bearer ' + credentials.t
...
const createRazorpayOrder = async (credentials, amount) => {
  try {
    let response = await fetch('/api/razorpay/order', {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
